...
2=3) может содержать 0-2 знаков '-', которые не могут идти подряд
2=4) может содержать 1 пару скобок '(' и ')'  , причем если она есть, то она расположена левее знаков '-'
2=5) скобки внутри содержат четко 3 цифры
6) номер не содержит букв
2=7) номер заканчивается на цифру
...
public class Solution {
    ...
    private static boolean checkNumberByPlus(String telNumber){
        if (telNumber.contains("+")){
            boolean rule1 = telNumber.matches("^\\+.+");
            boolean rule12 = (12 == getQuantityOfNumbers(telNumber));
            return rule1 && rule12;
        }
        return true;
    }
